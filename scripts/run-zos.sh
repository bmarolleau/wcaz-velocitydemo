...
if [ -z "$3" ]; then
  ...
  DEBUG_OPTIONS="-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=10.3.58.61:8000"
fi
set -ex
echo "Uploading jar file.."
zowe $FILES_CMD upload file-to-uss $LOCAL_PROGRAM \"$REMOTE_PROGRAM\" --binary $PROFILE --reject-unauthorized false
echo "Starting Java application.."
zowe $COMMAND_CMD "java $DEBUG_OPTIONS -jar \"$REMOTE_PROGRAM\" $INPUT1" $PROFILE --reject-unauthorized false
